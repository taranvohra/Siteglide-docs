# How to check whether a Category has children
...
Firstly, let's assign an array that will store all of the Category's children- after we've done this we'll loop over all the Categories and check whether they should be added to the array: \`
...
{%- if this_category_children.size > 0 -%}
  {% comment %}OUTPUT CHILDREN{% endcomment %}
{%- else -%}
  {% comment %}DO SOMETHING ELSE{% endcomment %}
{%- endif -%}
{% endraw %}
```
